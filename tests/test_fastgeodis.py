...
def fastgeodis_generalised_geodesic_distance_3d(image, softmask, v, lamb, iter, spacing):
    return FastGeodis.generalised_geodesic3d(image, softmask, spacing, v, lamb, 1-lamb, iter)
...
class TestFastGeodis(unittest.TestCase):
    ...
    def test_zeros_input(self, device, num_dims, base_dim):
        ...
        np.testing.assert_allclose(np.zeros(mask_shape, dtype=np.float32), geodesic_dist.cpu().numpy())
    ...
    def test_mask_ones_input(self, device, num_dims, base_dim):
        ...
        np.testing.assert_allclose(np.ones(mask_shape, dtype=np.float32) * 1e10, geodesic_dist.cpu().numpy())
